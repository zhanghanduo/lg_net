...
from lg_net.utils.exceptions import DataUnavailableError
import tarfile
...
class ImageNet(TorchVisionDataset):
    ...
    def __init__(self, *args, **kwargs):
        ...
        cache_file = self.root / self.__cache_file
    ...
    def download(self):
        if (self.root / self.__download_train_payload).exists() and (
            self.root / self.__download_val_payload
        ).exists():
        ...
        raise DataUnavailableError(
            ...
            f"     {self.root / self.__download_train_payload}\n"
            f"     {self.root / self.__download_val_payload}\n"
            f"     {self.root / self.__download_devkit_payload}\n"
        )